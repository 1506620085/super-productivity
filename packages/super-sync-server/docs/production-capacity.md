...
**状态：** 现行。**数字上次验证：** 2026-08-28；崩溃重启相关数字 2026-09-03。下文每个数字均为某一时点的测量——在依赖其中任何一个之前请重新测量。
...
**2. 数据库崩溃重启（已在 #9917 修复，平台会放大触发）。**
数月来某个「server process」大约每两周以退出码 2 退出，后来变为数小时一次（50 次，#9695）。这长期被读作 `quickdie` / `SIGQUIT` 路径，因而被视为无法从 OpenVZ 访客内归因。两者都不成立：死去的 PID 是 compose healthcheck 的 `pg_isready`——在 dockerd 杀死超时探测 shell 后成为孤儿，被作为 PID 1 运行的 postmaster 收养，并以退出码 2（「无响应」）退出。postmaster 会把任何非 0/1 的回收状态当作后端崩溃。已通过在 postgres 服务上设置 `init: true` 修复（见 `docker-compose.yml` 中的注释）；结案跟踪于 #9695。平台贡献的是触发条件：使探测超时的 25–52 秒 I/O 停顿，正来自此处描述的 150-IOPS 预算。
