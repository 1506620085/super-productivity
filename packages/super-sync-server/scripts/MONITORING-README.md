...
*过度*预测 — 它在 SQL 中镜像扫荡的跳过原因，但无法建模扫荡因数据库错误跳过的用户，而这正是
...
意味着*检查无法运行*的条件在部署时报告，从不进入告警正文，以免它们永久保持 `PROBLEMS`
...
若组已加好而 `journalctl -k` 仍打印空内容，则主机是根本不向访客暴露内核日志的容器（OpenVZ/Virtuozzo、LXC）——2026-09-09 在托管服务器上验证，cron 用户已在 `systemd-journal`，journal 仍空。没有任何组能修好这一点；该标记在那里是永久的，且 `docker inspect -f '{{.State.OOMKilled}}'` 是唯一的 OOM 信号。
...
*驱动*是另一回事，且刻意不声称恒定：`sync_devices` 在 `last_seen_at` 上没有索引，因此选择
