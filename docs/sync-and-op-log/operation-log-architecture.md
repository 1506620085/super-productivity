...
重放自起点起的*每一条*操作会太慢。我们使用 **Snapshots** 加速：

1.  **Load Snapshot：** 启动时，应用加载最新有效的 state-cache 快照。
2.  **Replay Tail：** 应用然后查询日志：「给我该快照*之后*发生的所有操作。」
...
- **Conflict Detection：** 因为每条操作都有 **Vector Clock**，我们可以数学证明两次变更是否并发。
  ...
  - _冲突：_ 若设备 B *也*做了变更且处于「Version 2」，它知道「等等，我们同时改了 Version 1！」-> **检测到冲突**。
...
在 500 次持久本地追加之后，客户端*尝试*压缩。当远程 reducer/归档工作、待处理本地写入、持久化
...
| 替代方案                          | 是什么                                   | 为何拒绝                                                                                                                                                                       |
| --------------------------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Last-Write-Wins（全局时间戳）** | 丢弃逻辑时钟；最新挂钟写入胜出           | 用户设备时钟不可靠；并发独立字段编辑会静默互相覆盖。对个人生产力应用不可接受。（仅作为冲突解决内的*字段级*决胜存活。）                                                         |
...
| **全状态 / 快照同步**             | 同步整个模型文件（旧 PFAPI 模型）        | 每次变更重新传输一切；无按实体冲突粒度；无法在离线编辑后重建意图。仅作为*引导*机制保留（快照 + 尾部重放），而非同步机制。                                                      |
| **CRDT（Yjs/Automerge 等）**      | 数学保证收敛                             | 概念复杂度高；多数实现假设可信服务器或中继，与哑文件 + E2EE 约束冲突。Op-log 刻意*借用*基于操作的 CRDT 属性（UUID 幂等、因果排序）而无完整机制。                               |
| **服务器分配序号**                | 让服务器强加全序                         | 排序需要服务器连通性——与离线优先及无服务器的基于文件的提供者不兼容。仅作为*补充*使用（SuperSync seq 用于全局顺序；向量钟仍为基于文件/离线情形所需）。                          |
...
版本升级仅栅栏在升级*之后*发布的接收方。截至 2026-07：
...
1. 新操作语义必须在旧客户端上优雅降级——见 `packages/sync-core` 中的 `LwwUpdatePayload` 信封模式（'patch' 操作通过 `updateOne` 在 pre-v3 客户端上正确应用；v4 delete-wins 标记对它们惰性）。若它们降级，在任何机群份额上升级都是*安全的*（戳是未来接收方的栅栏，而非当前接收方的保护）——但安全 ≠ 必要：若它降级，优先用标记/信封且**不**升级（见 0）。
...
#### 拓宽线缆词汇表无需 bump——但接收方必须阻塞

`opType`、`syncImportReason` 与恢复点 `type` 仅在 SuperSync 契约的 REQUEST 侧是严格枚举（服务器按 op 校验）。RESPONSE 侧它们是宽松字符串，由接收方逐条决定（`remote-op-block.util.ts`，由 `remote-ops-processing.service.ts`、全状态冲突门的前缀截断与 USE_REMOTE 预检共享的同一阻塞谓词）：未知值在该 op 处阻塞批次，行为恰如 `VERSION_TOO_NEW`——应用前缀、冻结游标、「请更新应用」snack——绝不可 skip（那会把游标推过客户端从未理解的数据）。（未知类型的恢复点仅留在列表中并以通用方式渲染。）此前传输解析器会以泛型错误拒绝整页，卡住每一个尚未更新的设备（#8764）。两点后果：

- 向这些联合类型添加值不需要 schema bump；按上文规则 0，不要为此升级。
- 在此接收方变更发布之前的客户端仍会在任何拓宽上卡住，因此拓宽必须等发布滞后期。窗口也覆盖服务器：`validation.service.ts` 在上传时拒绝未知 op 类型，因此自托管服务器必须先带上新值。

`super-sync.ts` 中的类型级断言保持 sync-core 的 `OpType` 枚举与 `SUPER_SYNC_OP_TYPES` 一致，因此只改一侧会在构建期失败，而不是在其他设备上表现为运行时阻塞。
...
剪枝/重置时钟可使*导入后*操作比较为并发。分类器因此保留两种可证明情形：来自导入自身客户端的更高计数器，或携带至少导入客户端边界计数器的操作。这些是因果证明，而非时间戳猜测。最新边界按持久批次/存储顺序选择，绝不用 UUID 顺序。
