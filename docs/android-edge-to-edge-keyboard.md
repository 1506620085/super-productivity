...
## 在 `<html>` 上写入 `--keyboard-height` 的代价（#9779，测量于 2026-09）

**在把任何 iOS 键盘修复移植到 Android 路径之前，先读本节。**
上方 Android/web 跟踪器把 `--keyboard-height` 写在 `document.documentElement` 上。
该写入并非免费——根上的自定义属性会由样式引擎按元素重新解析，因此代价随已渲染任务行数放大。
这正是 #9779（「在 iOS 上打开添加任务栏很卡，且屏幕上任务行越多越卡」）。

中位数 ms/次写入，201 行任务，iPhone 13 视口，`ng serve` 构建，WebKit 26.5 / Chromium 149：

| 写入                              | WebKit | Blink |
| --------------------------------- | ------ | ----- |
| 根上**自定义**属性                | ~180   | 12.8  |
| 根上**继承型标准**属性（`color`） | 0.1    | 0.3   |
| 叶子元素上的自定义属性            | 0.1    | 0.1   |
| 根自定义属性，行 `display: none`  | 8      | 1.6   |
| 空列表                            | ~9     | 0.7   |

由此得出三点，第三点是决策：

1. **特指自定义属性，而非继承属性。** 在 `<html>` 上写 `color` 会使同一批元素失效，但代价仍贴底。「别在根上写继承属性」是错误教训。
2. **只有根上的自定义属性写入会随行数放大。** shell 高度写入及其他测量项在 0–201 行间基本持平。
3. **Blink 对同一机制大约便宜一个数量级。** iOS 已在 [#9926](https://github.com/super-productivity/super-productivity/pull/9926) 中通过把变量移到 CDK overlay 容器（`IosKeyboardService`）修复。该修复**故意未移植到 Android 路径**：约 12.8ms/次、且频率约为每次打开键盘一次而非每动画帧一次，不值得增加间接层。若 Android 跟踪器开始按帧写入，或行数远超 200，再重新评估。

引用这些数字前的注意：WebKit 数字的次间波动约 16%，因此把低于约 1.2× 的差异视为未定论。用 `e2e/measure/ios-keyboard-relayout.measure.ts` 重测（手工运行，不在 `npm run e2e` 中——见其头部说明方法与 caveat）。
